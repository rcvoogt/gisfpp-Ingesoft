...
public class GestorTareasImp implements GestorTareas {
	 ...
	 *            (String)
	 * @param ordenadoPor
	 *            (String): criterio por el cual se va a ordenar el listado.
	 * @param asc
	 *            (boolean): determina si el modo de ordenación del listado es en
	 *            forma ascendente. "true" si se desea ordenar en forma ascendente,
	 *            "false" si se desea ordenar de forma descendente.
	 * @return Lista de tareas ordenada según los parámetros "ordenadoPor" y "asc".
	 * @throws GisfppWorkflowException
	 */
	@Override
	public List<InfoTarea> getTareasAsignadas(String usuario, String ordenadoPor, boolean asc)
			throws GisfppWorkflowException {
		...
		try {
			...
			throw new GisfppWorkflowException(
					"Se ha generado un error al intentar obtener el listado de tareas asignadas"
	...
	/**
	 * Devuelve el listado de tareas de las cuales el usuario pasado como parametro
	 * es candidato para su tratamiento.
	 * 
	 * @param usuario
	 *            (String)
	 * @param ordenadoPor
	 ...
	 */
	@Override
	public List<InfoTarea> getTareasPropuestas(String usuario, String ordenadoPor, boolean asc)
			throws GisfppWorkflowException {
		TaskQuery query = taskService.createTaskQuery();
		...
		try {
			...
			throw new GisfppWorkflowException(
					"Se ha generado un error al intentar obtener el listado de tareas propuestas"
							+ "para un usuario candidato específico.",
					exc1);
		}
	 ...
	 */
	@Override
	public List<InfoTarea> getTareasDelegadas(String usuario, String ordenadoPor, boolean asc)
			throws GisfppWorkflowException {
		TaskQuery query = taskService.createTaskQuery();
		List<Task> resultadoQuery = null;
		try {
			...
			switch (ordenadoPor) {
			case "porFechaVencimiento": {
				resultadoQuery = (asc == true) ? queryParcial.orderByDueDateNullsLast().asc().list()
						: queryParcial.desc().list();
				break;
			}
			case "porPrioridad": {
				resultadoQuery = (asc == true) ? queryParcial.orderByTaskPriority().asc().list()
						: queryParcial.orderByTaskPriority().desc().list();
				break;
			}
			case "porNombreTarea": {
				resultadoQuery = (asc == true) ? queryParcial.orderByTaskName().asc().list()
						: queryParcial.orderByTaskName().desc().list();
			}
			}
		} catch (Exception exc1) {
			throw new GisfppWorkflowException(
					"Se ha generado un error al intentar obtener el listado de tareas delegadas"
							+ "para un usuario específico.",
					exc1);
		}
		if (resultadoQuery != null && !resultadoQuery.isEmpty()) {
	...
	/**
	 * Devuelve el listado de tareas ya realizadas por el usuario pasado como
	 * parámetro.
	 * 
	 * @param usuario
	 *            (String)
	 * @return Lista de tareas concluidas, ordenadas por fecha de conclusion en
	 *         forma descendente.
	 */
	@Override
	public List<InfoTarea> getTareasRealizadas(String usuario) throws GisfppWorkflowException {
		...
		try {
			...
			throw new GisfppWorkflowException(
					"Se ha generado un error al intentar obtener el listado de tareas realizadas"
							+ "para un usuario específico.",
					exc1);
		}
		if (resultadoQuery != null && !resultadoQuery.isEmpty()) {
	...
	public List<InfoTarea> getTareas(String proceso, boolean asc) {
		ProcessDefinition processDefinition = repoService.getProcessDefinition(proceso);
		List<Task> resultadoQuery;
		TaskQuery query = taskService.createTaskQuery();
		resultadoQuery = query.processDefinitionKey(processDefinition.getKey()).list();
	...
	private InfoTarea convertirTarea(TaskInfo tarea){
	...
	public void reclamarTarea(InfoTarea tarea, String usuario) throws GisfppWorkflowException {
		try {
			...
			throw new GisfppWorkflowException(
					"Se ha producido un error al intertar reclamar la tarea. \n Mensaje: " + exc3.getMessage(), exc3);
		}
	...
	public void delegarTarea(InfoTarea tarea, String usuarioDestino) throws GisfppWorkflowException {
		try {
			...
			throw new GisfppWorkflowException(
					"Se ha producido un error al intertar delegar la tarea. \n " + "Mensaje: " + exc2.getMessage(),
					exc2);
		}
	...
	private void completarTarea(InfoTarea tarea) throws GisfppWorkflowException {
		try {
			...
			throw new GisfppWorkflowException("La tarea con Id. " + tarea.getId() + " que intenta completar no existe.",
					exc1);
		} catch (Exception exc2) {
			throw new GisfppWorkflowException(
					"Se ha producido un error al  intentar completar la tarea. " + "\n. Mensaje: " + exc2.getMessage(),
					exc2);
		}
	}

	private void completarTarea(InfoTarea tarea, Map<String, Object> variables) throws GisfppWorkflowException {
		try {
	...
	private void resolverTarea(InfoTarea tarea) throws GisfppWorkflowException {
		try {
			...
			throw new GisfppWorkflowException(
					"Se ha producido un error al intertar resolver la tarea. \n" + " Mensaje: " + exc2.getMessage(),
					exc2);
		}

	}

	private void resolverTarea(InfoTarea tarea, Map<String, Object> variables) throws GisfppWorkflowException {
		try {
	...
	public long getCantidadPorEstado(String usuario, EstadosTarea estado) throws GisfppWorkflowException {
		...
		try {
			...
			throw new GisfppWorkflowException(
					"Se ha producido un error al consultar la cantidad de tareas según el tipo" + " especificado.",
					exc);
		}
	}
